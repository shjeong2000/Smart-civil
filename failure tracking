class Beam:
    def __init__(self, length, width, height, beam_type):
        ...
        self.beam_type = beam_type  # 보의 종류 (단순보, 캔틸레버 보 등)
...
class StressTensor:
    ...
    def shear_stress(self, shear_force, y, z):
        Q = (self.beam.width * (self.beam.height / 2 - y) * y) / 2
...
def main():
    beam_type = input("보의 종류를 입력하세요 (단순보, 캔틸레버 보, 연속보, 내민보, 고정보): ")
    length = float(input("보의 길이 (m): "))
    width = float(input("보의 너비 (mm): "))
    height = float(input("보의 높이 (mm): "))
    beam = Beam(length, width, height, beam_type)
    ...
    while True:
        load_type = input("추가할 외력 종류를 선택하세요 (point_load, point_moment, distributed_load, 종료하려면 'exit'): ")
    ...
    x_position = float(input("내력을 계산할 위치 x (m): "))
    y_position = float(input("내력을 계산할 위치 y (mm): ")) / 1000
    z_position = float(input("내력을 계산할 위치 z (mm): ")) / 1000
    ...
    tensor = stress_tensor_calculator.stress_tensor(x_position, y_position, z_position, -700, chosen_moment, chosen_shear)
    print("응력 텐서:")
    for row in tensor:
